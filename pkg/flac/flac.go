...
import (
	...
	"github.com/drgolem/go-flac/pkg/ringbuffer"
)
...
func (d *FlacDecoder) DecodeSamples(samples int, audio []byte) (int, error) {
	...
	for samplesToRead > 0 {
		...
		available := d.ringBuffer.Size()
		availableSamples := available / (d.channels * d.outputBytesPerSample)
		...
		if state == C.FLAC__STREAM_DECODER_END_OF_STREAM || availableSamples >= samplesToRead {
			...
			if bytesToRead > 0 {
				n, err := d.ringBuffer.Read(bytesToRead, audio[offset:])
				if err != nil {
